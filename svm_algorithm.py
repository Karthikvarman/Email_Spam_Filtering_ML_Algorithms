...
data['Predicted'] = svm_model.predict(data_tfidf)
...
svm_cm, svm_report = evaluate_model(svm_model.predict(X_test_tfidf), y_test, "SVM")
...
svm_values = [
    ...
    precision_score(y_test, svm_model.predict(X_test_tfidf)),
    recall_score(y_test, svm_model.predict(X_test_tfidf)),
    f1_score(y_test, svm_model.predict(X_test_tfidf))
]

metrics_df = pd.DataFrame({"Metric": metrics, "Score": svm_values})
